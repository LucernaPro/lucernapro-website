...
{fig(5,'ช่างนั่งยองใช้แปรงทา DeepSeal แนวผนังชนพื้นใต้แนวหน้าต่างกระจกภายในองค์เจดีย์','แนวผนังชนพื้นใต้แนวหน้าต่าง — มุมฉากที่ผนังตั้งชนพื้นราบคือรอยต่ออันดับต้นๆ ที่เรารู้จักดี')}
    ...
    <section class="step">
      ...
      <p>ทีมช่างชุดเดียวกันโรยตัวลงไปทาตามแนวรอยต่อกระเบื้องด้านนอกองค์เจดีย์ด้วย ภาพสวยและงานเสี่ยงมาก — แต่เราจะไม่เขียนเชียร์ส่วนนี้ เพราะ<b>เราไม่แนะนำให้ใช้ DeepSeal กลางแจ้งแบบเปลือย</b> ฟิล์มระบบนี้โดนแดดสะสมนานๆ จะเหลืองขึ้น (ความแข็งแรงไม่ลด แต่หน้าตาเปลี่ยน) และมันถูกจูนมาเพื่องานภายในโดยเฉพาะ</p>
      <p><b>ถ้าจำเป็นต้องใช้กลางแจ้งจริงๆ กติกามีข้อเดียว: ต้องทาสีหรือกันซึมทับเสมอเพื่อกันแดด</b> ทาทับได้ตามปกติเมื่อแห้งสนิท — ปล่อยเปลือยไว้คือเอาของที่เกิดมาเพื่อในร่ม ไปยืนตากแดดทั้งปี</p>
...
{fig(5,'Worker crouching, brushing DeepSeal along the wall-to-floor line below a run of windows inside the chedi','The wall-to-floor line below the windows — the right-angle where a wall meets a floor is one of the joints we know best')}
    ...
    <section class="step">
      ...
      <p>The same crew also abseiled down the outside and brushed along the tile joints of the chedi. Striking photos, and dangerous work — but we will not cheer this part, because <b>we do not recommend DeepSeal outdoors left bare</b>. This film yellows with long UV exposure (strength unchanged, appearance not), and it is tuned specifically for interior work.</p>
      <p><b>If it must be used outdoors, there is one rule: always paint or waterproof over it to keep the sun off.</b> It takes a topcoat normally once fully dry — leaving it bare means taking a product born for indoor work and standing it in the sun all year.</p>
