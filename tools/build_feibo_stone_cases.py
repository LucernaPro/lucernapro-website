...
CASES.append(dict(slug=s, ngal=4,
 th=dict(title="ผนัง GRC ศูนย์ศิลปะเหมยซีหู — ผนังขาวโค้งที่เจ้าของไม่ยอมให้เหลือง",
  ...
  steps=[
   ("01","แผง GRC เดียวกัน ทำครึ่ง ไม่ทำครึ่ง",["ฝั่งหนึ่งทำระบบ 2 ชั้นแล้ว อีกฝั่งยังไม่ได้ทำ แล้วโดนแดดฝนมาด้วยกัน — ภาพนี้ไม่ได้ล้าง ไม่ได้แต่ง เส้นแบ่งคือขอบเขตที่ทำ"],[(2,"ผนัง GRC ฝั่งซ้ายเคลือบระบบ 2 ชั้นขาวสะอาด ฝั่งขวาไม่เคลือบเหลืองหม่นมีคราบน้ำไหล","ซ้าย: ทำแล้ว ขาวเรียบ · ขวา: ไม่ทำ เหลืองหม่นและคราบน้ำไหลเป็นทาง — GRC ต่อเนื่องชิ้นเดียวกัน")]),
 ...
 en=dict(title="The GRC Facade of Meixihu Arts Centre — Curved White Walls the Owner Would Not Let Turn Yellow",
  ...
  steps=[
   ("01","Same GRC panel, half treated, half not",["One side had the two-layer system, the other not yet, and both took the same weather — this photo is not washed or retouched; the line is the edge of the treated area."],[(2,"GRC wall, left side coated with the two-layer system and clean white, right side uncoated, yellowed with water streaks","Left: treated, smooth white · Right: untreated, dull yellow with water streaks — one continuous piece of GRC")]),
...
CASES.append(dict(slug=s, ngal=5,
 th=dict(title="ผนังหินแกรนิตอาคารพักอาศัย — ซุ้มทางเข้าและป้ายชื่อ จุดที่คราบไหลลงเห็นชัดก่อนจุดอื่น",
  desc="โครงการของ Feibo บนผนังหินแกรนิตของอาคารพักอาศัยระดับบน — ทำระบบ 2 ชั้นที่ซุ้มทางเข้าและป้ายชื่ออาคาร ด้วยรถกระเช้าขากรรไกรและนั่งร้าน โดยหินยังเป็นสีเดิม",
  ...
  steps=[
   ...
   ("02","เจ้าของงานมีเงื่อนไขเดียว: หินต้องเป็นสีเดิม",["นี่คือเหตุผลที่ระบบหินต้องใส ไม่เงา ไม่เปลี่ยนสี — งานอาคารระดับนี้ เจ้าของมักรับไม่ได้ถ้าผนังหินดูเป็นพลาสติก ขั้นตอนมาตรฐานคือทดสอบมุมเล็กให้เห็นก่อนว่าแกรนิตขัดหยาบสีนี้ไม่เข้มขึ้น แล้วจึงลงทั้งซุ้ม"],[(4,"ช่างบนรถกระเช้าเคลือบใต้ขอบซุ้มทางเข้าอาคารหินแกรนิต ระยะใกล้","ขอบซุ้ม — จุดที่น้ำฝนจากด้านบนไหลตกแล้วทิ้งคราบเป็นแนว ทำให้ครบทั้งหน้าและใต้ขอบ")]),
 ...
 en=dict(title="Granite Facade of a Residential Tower — the Entrance Portico and Name Sign, Where Run-off Shows First",
  desc="A Feibo project on the granite facade of an upmarket residential building — the two-layer system applied at the entrance portico and name sign from a scissor lift and scaffold, with the stone keeping its original colour",
  ...
  steps=[
   ...
   ("02","The owner had one condition: the stone stays the same colour",["This is why the stone system is clear, non-gloss and colour-neutral — on a building at this level the owner will not accept stone that looks like plastic. Standard practice is a small test patch to show that this flamed granite does not darken, and only then the whole portico."],[(4,"Applicator on a lift coating the underside of the entrance portico of a granite building, close view","The portico edge — where rain running off the top leaves a line of staining; treated on the face and under the edge")]),
...
CASES.append(dict(slug=s, ngal=4,
 th=dict(title="ราวหินอ่อนสลักลายหน้าสถานีรถไฟจี๋โส่ว — หินขาวที่ดำเร็วที่สุดเมื่อเจอฝุ่นถนน",
  desc="โครงการของ Feibo บนราวหินอ่อนขาวสลักลายหน้าสถานีรถไฟจี๋โส่ว — ล้างพื้นหินรอบราวด้วยเครื่องขัด ล้างราวด้วยมือ แล้วลงระบบ 2 ชั้นทีละช่วง ให้ลวดลายสลักลึกได้รองพื้นซึมถึงทุกร่อง",
  ...
  steps=[
   ("01","ล้างก่อน — ระบบนี้ป้องกัน ไม่ใช่ล้าง",["คราบดำและตะไคร่ที่ฝังในหินอ่อนต้องออกก่อน ไม่งั้นรองพื้นซึมไม่เข้าและคราบจะถูกล็อคไว้ใต้ระบบ พื้นหินรอบราวใช้เครื่องขัดกับน้ำยาล้างหิน ส่วนราวสลักล้างด้วยมือ แล้วรอให้หินแห้งสนิทก่อนลงชั้นแรก"],[(2,"ช่างใช้เครื่องขัดล้างพื้นหินรอบราวหินอ่อนหน้าสถานีรถไฟ ก่อนเคลือบ","ขั้นล้าง — เครื่องขัดล้างพื้นหินรอบราว ราวสลักล้างด้วยมือ ขจัดคราบดำที่ฝังในผิวก่อน หินต้องแห้งสนิทก่อนลงรองพื้น")]),
 ...
 en=dict(title="Carved Marble Balustrades at Jishou Railway Station — the White Stone That Goes Black Fastest in Road Dust",
  desc="A Feibo project on the carved white marble balustrades outside Jishou railway station — the stone floor around it machine-cleaned and the balustrade cleaned by hand, then the two-layer system applied section by section so the primer reaches every groove of the carving",
  ...
  steps=[
   ("01","Clean first — this system protects, it doesn't clean",["Black staining and algae already in the marble have to come out first, or the primer can't penetrate and the stain is locked under the system. The stone floor around the balustrade was done with a floor machine and stone cleaner, the carved balustrade by hand, and the stone was left to dry fully before the first layer."],[(2,"Applicator using a floor machine to clean the stone floor beside a marble balustrade outside a railway station before coating","The cleaning step — floor machine for the stone floor, the carved balustrade by hand, lifting the black staining first; the stone must be completely dry before priming")]),
...
CASES.append(dict(slug=s, ngal=6,
 th=dict(title="ลานกลองเซียงซี — หินสลัก ราวสะพาน และประติมากรรมกลางแจ้งที่ต้องอยู่ได้เอง",
  desc="โครงการของ Feibo ในลานสาธารณะที่เซียงซี — หินสลักรูปมังกร ฐานประติมากรรมกลอง และราวหินสะพานยาวทั้งแนว ทำระบบ 2 ชั้นให้ร่องสลักลึกไม่เก็บตะไคร่และไม่ต้องล้างแรงดันสูงเป็นรอบๆ",
  intro=[DISC_TH, "ลานสาธารณะคือหน้างานที่ไม่มีใครดูแลรายวัน — หินสลัก ราวสะพาน ฐานประติมากรรม โดนฝน ฝุ่น นก และมือคนตลอดปี แล้วก็ถูกล้างด้วยแรงดันสูงเป็นรอบๆ ซึ่งกัดผิวหินและทำให้ลายสลักจางไปเรื่อยๆ"],
  ...
  steps=[
   ...
   ("02","ราวหินสะพาน — ยาวทั้งแนว ไล่ทำทีละช่วง",["ราวหินสลักลายทั้งสองฝั่งสะพาน ทีมงานจอดรถตู้อุปกรณ์ไว้ข้างทางเป็นจุดพักของ แล้วเดินไล่ทำบนทางเท้าทีละช่วง กั้นทางเดินสั้นๆ ให้คนยังเดินผ่านได้ ราวแบบนี้คือสิ่งที่มักถูกล้างด้วยแรงดันสูงเป็นรอบๆ — หลังทำระบบ ฝนล้างให้เอง"],[(3,"ช่างเคลือบราวหินสะพาน มีรถตู้อุปกรณ์จอดข้าง","รถตู้อุปกรณ์จอดข้างทางเป็นจุดพักของ ทีมงานเดินไล่ทำบนทางเท้าทีละช่วง — กั้นทางเดินสั้นๆ คนยังเดินผ่านได้"),(4,"ราวหินสลักลายบนสะพานในเมือง","ราวสะพานยาวทั้งแนวหลังทำ — ลายสลักยังคม และไม่ต้องโดนแรงดันสูงซ้ำๆ อีก")]),
 ...
 en=dict(title="Xiangxi Drum Plaza — Stone Carvings, Bridge Balustrades and Outdoor Sculpture That Have to Look After Themselves",
  desc="A Feibo project in a public plaza in Xiangxi — a carved stone dragon, the base of a drum sculpture and a full run of stone bridge balustrade, treated with the two-layer system so the deep carving stops collecting algae and the repeated pressure-washing stops",
  intro=[DISC_EN, "A public plaza is the site nobody looks after daily — carvings, balustrades and sculpture bases take rain, dust, birds and hands all year, and then get pressure-washed round after round, which eats the stone surface and wears the carving down a little more each time."],
  ...
  steps=[
   ...
   ("02","The bridge balustrade — the whole run, section by section",["Carved stone balustrade on both sides of the bridge: the crew parked the equipment van at the kerb as a supply point and worked along the pavement section by section, coning off short stretches so pedestrians could still pass. This is exactly the kind of thing that gets pressure-washed round after round — after the system, the rain does it."],[(3,"Applicator coating a stone bridge balustrade with the equipment van parked alongside","Equipment van parked at the kerb as the supply point, the crew working along the pavement section by section — short stretches coned off, pedestrians still passing"),(4,"Carved stone balustrade along a city bridge","The full run of balustrade after treatment — the carving stays crisp, and it no longer has to face a pressure washer round after round")]),
